...
if($_SERVER['REQUEST_METHOD']==="POST"){//ОЧЕНЬ ВАЖНЫЙ МОМЕНТ - POST НУЖНО ПИСАТЬ КАПСОМ А НЕ МАЛЕНЬКИМИ БУКВАМИ
    ...
    $comment_id=$_POST["comment_id"];
        ...
        try {
            ...
            // }
            if(empty($comment_id)){
                $errors["comment_id_null"]= "Вы забыли указать ID комментария, который хотите скрыть!";
            }
            ...
            if ($errors) {
                ...
                $_SESSION["errors_commentban_find"]=$errors;
            ...
            if ($res["comment_activated"]==0) {
                $errors["deactive_true"]="Комментарий пользователя ".strtoupper($res["username"])." уже скрыт!";
            }
            ...
            deactivation_comment($pdo, $comment_id);
            header("Location: ../moder_page.php?commentban=success");
